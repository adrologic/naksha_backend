...
function resolveConfig(): AuthConfig | null {
  ...
  return { email, passwordHash, secret, ttlMs: env.AUTH_TOKEN_TTL_HOURS * 60 * 60 * 1000 };
}

export const authConfig = resolveConfig();
...
if (!authConfig) {
  ...
  console.warn(
    "[auth] Admin auth is NOT configured — set ADMIN_EMAIL, AUTH_SECRET and " +
      "ADMIN_PASSWORD_HASH (or ADMIN_PASSWORD). Until then /auth/login returns 503 " +
      "and all write endpoints reject with 401.",
  );
}
